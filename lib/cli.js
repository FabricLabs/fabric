...
class CLI extends Fabric {
  ...
  async _loadHistory () {
    ...
    for (let i in logs) {
      let message = await cli._GET(`/messages/${logs[i]}`);
      if (!message) {
  ...
  async _handleSubmit (data) {
    ...
    if (data.input.charAt(0) === '/') {
      ...
      let blob = await this._GET(result);
  ...
  trust (source) {
    ...
    source.on('changes', async function (changes) {
      ...
      cli.log('result from cli._applyChanges:', state.id);
      cli.log('cli[@data].messages:', cli['@data'].messages);
    });

    // TODO: internalize to CLI
    // TODO: fix route -- `channels/messages`
    source.on(`channels/~1messages`, function (event) {
      cli.log('deep channel event on "channels/~1messages": ', event['@data']);
  ...
  _appendMessage (message) {
    ...
    if (self.elements.history) {
      self.elements.history.pushLine(`${instance.created}${(instance.actor) ? ' ' + instance.actor : ''}: ${instance.input}`);
