...
CLI.prototype._handlePeerMessage = function (message) {
  ...
  self.peers.pushLine(`${instance.created}${(instance.actor) ? ' ' + instance.actor : ''}: ${instance.input}`);
  self.peers.setScrollPerc(100);
...
CLI.prototype._appendLogMessage = function (message) {
  ...
  self.logs.pushLine(`${instance.created}${(instance.actor) ? ' ' + instance.actor : ''}: ${instance.input}`);
