...
class CLI extends Fabric {
  ...
  async _handleSubmit (data) {
    ...
    if (data.input.charAt(0) === '/') {
      ...
      let vector = new Vector({
        ...
        parent: this.oracle.state.id,
        integrity: `sha256:${state.id}`,
        input: data.input
      });

      // TODO: visual indicator of "sending..." status
      let result = await this.oracle._POST('/messages', vector['@data']);
      let target = await this.oracle._GET(`/messages/${vector.id}`);
      this.oracle.on('changes', function (changes) {
        self.log('manual bypass, changes:', changes);
      })
      let collection = await this._GET(`/messages`);
  ...
  async start () {
    ...
    swarm.on('changes', async function (changes) {
      ...
      await self.oracle.machine.applyChanges(changes);
      await self.oracle.machine.commit();
    ...
    let logs = await self._GET('/messages') || [];
