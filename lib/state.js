...
`;
  ...
  deserialize (input) {
    ...
    if (typeof input === 'string') {
      ...
      try {
        ...
        console.error('Could not parse string as Buffer:', E);
      }

      return output;
    } else {
      console.log('WARNING:', `input not a string`, input);
    }
  ...
  render () {
    ...
    switch (this['@type']) {
      default:
        console.log('[RENDER]', 'unhandled type:', this['@type']);
        console.log('[RENDER]', 'unhandled data:', this['@data']);
        break;
      case 'Array':
