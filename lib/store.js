...
class Store extends Scribe {
  ...
  async _REGISTER (obj) {
    ...
    this.log('registering:', obj);
    this.log('state:', vector);
  ...
  async _POST (key, value) {
    ...
    console.log('HEADS UP:', 'creating collection:', key, list);
  ...
  async get (key) {
    ...
    let hash = null;
    let data = null;
    let path = this.sha256(pointer.escape(key));
    let instance = null;
    ...
    if (this['@data'].states) {
      console.log('[CACHE]', 'states:', this['@data'].states);
      if (this['@data'].states[path]) return this['@data'].states[path];
    }

    try {
      console.log('testing...');
      let test = await self.state['@data'].collections[key];
      console.log('test:', test);
      if (test) {
        core = await Promise.all(test['@data'].map((x) => {
          console.log('x:', x);
          return self._GET(`/states/${x}`);
        }));
    ...
    if (item) {
      switch (type) {
        default:
          this.error('Unhandled type:', key, type, hash, item);
          break;
        case 'Buffer':
  ...
  async set (key, value) {
    ...
    let id = this.sha256(pointer.escape(key));
    ...
    console.log('content:', content.constructor.name, content);
    ...
    console.log('raw:', raw);
    console.log('pure state id:', pure['@state'].id);
    console.log('pure type:', pure['@type']);

    // first store the type
    try {
      await this.db.put(`/types/${pure['@state'].id}`, pure['@type']);
      pointer.set(this['@data'], `/types/${pure['@state'].id}`, pure['@type']);
    } catch (E) {
      this.error(E);
    }
    ...
    try {
      ...
      pointer.set(this['@data'], blob, raw);
    } catch (E) {
      this.error(E);
    }

    console.log('setting address:', address, value);

    try {
      await this.db.put(address, value);
    ...
    try {
      ...
      this.error('Could not set state:', E);
    }
    ...
    try {
      // console.log('before, db:', this.db);
      let output = await this.db.put(`/tips/${id}`, state.id);
      // console.log('output:', output);
      // TODO: migrate to Chain
      pointer.set(this['@data'], `/tips/${id}`, state.id);
      // console.log('state date:', this['@data']);
    } catch (E) {
      this.error(E);
    }
  ...
  trust (source) {
    ...
    source.on('put', function (key, value) {
      store.log('[TRUST:SOURCE]', source.constructor.name, 'emitted a put event', source, name);
