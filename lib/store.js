...
class Store extends Scribe {
  ...
  async _PATCH (key, patch) {
    console.log('[STORE]', '_PATCH', 'patch:', key, typeof patch, patch);
    let root = {};
  ...
  async _POST (key, value) {
    ...
    try {
      if (entity) {
        family = await self.populate(JSON.parse(entity));
      ...
      let answer = await self.db.put(address, origin.serialize());
    } catch (E) {
