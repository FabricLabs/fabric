...
class Store extends Scribe {
  ...
  async get (key) {
    ...
    try {
      blob = await self.db.get(`/tips/${path}`);
    } catch (E) {
      this.warn(`Could not retrieve key ${key}:`, E);
    }

    try {
      data = await self.db.get(`/blobs/${blob}`);
    } catch (E) {
      this.warn(`Could not retrieve key ${key}:`, E);
    }
    ...
    try {
      ...
      console.warn(`Could not parse data "${data}":`, E);
    }

    let state = new State(data['@data']);
    let pure = { '@id': state.id, '@data': state['@data'], '@type': state['@data'].constructor.name };
    let raw = JSON.stringify(state['@data']);
