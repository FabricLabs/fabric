...
class Store extends Scribe {
  ...
  async _REGISTER (obj) {
    ...
    console.log('[STORE]', '_REGISTER', vector.id, vector['@type']);
    ...
    try {
      result = await this._SET(`/states/${vector.id}`, vector['@data']);
    } catch (E) {
      this.error('Error creating object:', E, obj);
    }
  ...
  async get (key) {
    ...
    console.log(`key: ${key} name: ${id} router: ${router}`, tip, type, collection);
  ...
  async set (key, value) {
    ...
    let result = await this.get(`/states/${pure['@id']}`).catch(E => this.error.bind(this));
  ...
  trust (source) {
    ...
    source.on('put', function (key, value) {
      ...
      pointer.set(store.state['@data'], `${name}/blobs/${router}`, value);

      store.log('source put, pointer set:', key, id, router, value);
      store.log('store[state]:', store.state['@data']);
      store.log('!!! sanity check:', store.state['@data'].sources);
