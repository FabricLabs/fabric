...
class Fabric extends Scribe {
  ...
  use (name, description) {
    ...
    let opcode = new Opcode(description);

    this.opcodes[name] = opcode;
    this.log('[FABRIC]', 'opcode:', opcode);
  ...
  trust (source) {
    ...
    source.on('user', function (user) {
      self.emit('user', {
        id: [source.name, 'users', user.id].join('/'),
        name: user.name,
    ...
    source.on('join', async function (join) {
      self.emit('join', {
        user: [source.name, 'users', join.user].join('/'),
    ...
    source.on('message', async function (msg) {
      ...
      let message = {
        ...
        actor: [source.name, 'users', msg.actor].join('/'),
