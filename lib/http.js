...
function HTTP () {
  ...
  this.router.get('/people', function (req, res) {
    return res.send(self.app);
  });
}

util.inherits(HTTP, require('./vector'));

HTTP.prototype.define = function (name, definition) {
  var resource = this.app.define(name, definition);
};
...
HTTP.prototype.start = function () {
  this.server = this.router.listen(3000, function () {
