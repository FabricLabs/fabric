...
class Service extends Scribe {
  ...
  async _POST (path, data, commit = true) {
    ...
    console.log('resulting collection:', this.collections[name]);
    console.log('resulting collection (populated):', await collection.populate());

    // TODO: reduce storage to references
    try {
      await this._PUT(path, await collection.populate());
      await this.set(path, collection);
