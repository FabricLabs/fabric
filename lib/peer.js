...
Peer.prototype._handleMessage = async function handler (packet) {
...
Peer.prototype._handleBasePacket = function handlePacket (packet) {
  let message = null;
...
Peer.prototype._broadcastTypedMessage = function message (type, message) {
  if (!message) message = '';
  if (typeof message !== 'string') message = JSON.stringify(message);
...
Peer.prototype.start = function start () {
  ...
  peer.machine.on('changes', function (changes) {
    peer.log('PEER', 'the peer (inner machine) emitted changes:', changes);
    peer.emit('test/changes', changes);
  });
