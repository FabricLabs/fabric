...
Resource.prototype.flush = async function () {
  return this.store.set(this.routes.query, []);
};
...
Resource.prototype.create = async function (obj) {
  ...
  let collection = await self.store.get(self.routes.query);
  let path = [self.routes.query, vector.id].join('/');
  ...
  try {
    ...
    await self.store.set(self.routes.query, collection);
  } catch (E) {
    console.error(E);
  }
...
Resource.prototype.get = async function (id) {
  let self = this;
  let instance = await self.store.get([self.routes.query, id].join('/'));
