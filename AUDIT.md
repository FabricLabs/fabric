...
10. **Public API short names** — `contractId` and similar remain until a coordinated rename to `contractIdentifier`-style identifiers (Author Style in `AGENTS.md`). Prep: `@fabric/core/functions/contractIdentifier` dual-read/`stamp` (does not rewrite wire schemas).
