# Fabric Quickstart
For production deploys and native dependencies, see [`docs/PRODUCTION.md`](docs/PRODUCTION.md).  Contributor-oriented detail is in [`DEVELOPERS.md`](DEVELOPERS.md).
...
2. Run `fabric setup` (TTY: key TUI; new wallets are password-sealed — `--password` / `FABRIC_PASSWORD`. `--no-tui` / `--force` for scripts)
