...
**Outstanding (seals / accumulate):** tip-bound and participant AES-GCM seals bind scheme (+ tip / contract / ephemeral) as AAD. Remaining accumulate work: journal / re-fold caps ([docs/ARC.md](docs/ARC.md) §8). Genesis `bitcoinAnchor.blockHash` is only folded when it is 64-hex (same gate as `meta.bitcoinBlockHash`). Explicit tip `signers: []` no longer widens `tipSpendKeys` / `resolveSpend` to participant `members`.
...
**Blinded-execution decisions:** `recordProposalDecision` verifies BIP340 and treats an existing actor/proposal/decision tuple as idempotent (replays must not grow `session.decisions` or change `coordinationRoot`). Still not Yao GC.
...
**Still open (non-blocking for this PR pass):** eager `messageHex` on Peer hot paths; `contractId` → `contractIdentifier` rename; Hub/Passport callers still on hard-coded `7778` until they adopt `fabricIdentityDerivationPath`; `API.md` index sync via `make:api` for gossip / parse / `resolveSpend` option docs; journal / re-fold caps ([docs/ARC.md](docs/ARC.md) §8). Blinded-execution remains a scaffold (not Yao GC) even with signed decisions + hashlock+pubkey leaves — `recordProposalDecision` still does not bind `at` into `decisionSigningMessage` (deliberate until a coordinated protocol-string bump; see [AUDIT.md](AUDIT.md) PR #183 triage — **Medium** remaining on tip `51ad619c` security review). Beacon federation rounds stop collecting at `ready`; Hub should idempotently finalize already-`ready` rounds if epoch-chain persist fails ([docs/ARC.md](docs/ARC.md) §8 item 25). Peering still treats remote-advertised offer/announce pubkeys as a self-dial suppress hint (FIFO-capped); a verified binding (`verifiedPubkey` from session/registry) is the follow-up so a peer cannot poison dial targets by spoofing this node's key. OP_RETURN hallmarks ([docs/STATECHAIN.md](docs/STATECHAIN.md) / `functions/fabricHallmark`) are encode/verify in core; Hub opt-in publish+scan is separate — no P2P hallmark broadcast. Evaluator accepts for blinded-execution finalization exclude the garbler key.
