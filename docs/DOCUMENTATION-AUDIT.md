...
- **Live examples** — GitHub Pages (`https://fabriclabs.github.io/fabric/`, `examples/home.html`) is the demos deploy from `master` Pages actions. **`dev.fabric.pub/examples`** may lag; confirm after operator sync. Run `CHECK_BOOK_EXTERNAL=1 npm run check:book-links` after `npm run make:dev`.
