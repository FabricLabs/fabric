...
**Last reviewed:** 2026-09-06 — [#187](https://github.com/FabricLabs/fabric/pull/187) vs Hub production plan: **partially adequate** for Beacon ARC / registry / sidechain federation wire (fail-closed patches, contract publish authority, operator identity, Beacon sign rounds). **Not** a full production-plan close: WIP title, MuSig2 epoch aggregate still stub, Hub still owns redeploy pin + Origin-GET redeem + coordinated `contractIdentifier` rename. **Staged unifications (append):** shared `functions/keyHasPrivateSigningMaterial.js` (sidechain + execution-run builders); Peer drops duplicate `normalizePeerPubkeyHex` and uses `authoritySetHasPubkey` in `_signerMayPatchContract`; `fabricProgramManifest` requires `sidechainState` directly (no silent catch). Skip for this PR: Environment/CLI identity unify, Hub↔Peer federation policy parser, `contractIdentifier` call-site adoption, MuSig2 implement. Do not merge as “production complete.”

**Prior:** 2026-09-06 — CodeRabbit leftovers on tip `2eb343def` (watch-only `FABRIC_XPRV` before wallet; env federation threshold; typetree MODULE_NOT_FOUND-only; sidechain private-material guard).

**Prior:** 2026-09-03 — Two-week cut series **staged** (C1–C10): #187 tip + Codecov c8 whitelist for new authority/operator/MuSig2-stub modules; NOISE deps declared; unloadable modules excluded from `files[]`; type-tree inventory locked; `sensitive` ≠ encrypted docs; `contractIdentifier` dual-field helper (rename Blocker stays open). **Owner:** commit → open PRs → Bugbot/Security → Hub redeploy for NOISE P0 verify.
