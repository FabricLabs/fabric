...
**Last reviewed:** 2026-08-14. [#185](https://github.com/FabricLabs/fabric/pull/185) HEAD `5557a2bf` + RC1 first-tier + PR-review coverage (`tests/rc1.pr-review.coverage.js`; inventory cannot smuggle `type: 98`).
...
- JSON `type: 98` on a **P2P_PEERING_OFFER** AMP frame still dispatches as a peering offer. The same numeric type on **P2P_INVENTORY_RESPONSE** (or any other opcode) cannot enqueue a candidate. Locked in `tests/rc1.pr-review.coverage.js` with beat/isolate/NOISE-handler and `_fillPeerSlots` retry-map coverage from [#185](https://github.com/FabricLabs/fabric/pull/185) Codecov + review threads.
...
## PRs
[#185](https://github.com/FabricLabs/fabric/pull/185) — WIP ARC polish on `feature/rsi` after [#183](https://github.com/FabricLabs/fabric/pull/183) merge. Ubuntu **and** macOS tests **pass** on `5557a2bf`; local `npm test` after `report:install`: **2036 passing**, 4 pending. Codacy **SUCCESS** (home-env path-join excludes). Bugbot Highs (wrong cross-sign pubkey; duplicate outbound dials) addressed; `pubkey@host:port` now canonicalizes onto the same dial slot. `_disconnect` already calls `_teardownNoiseClient`. RC1 author-flip hashes the mutated buffer. `_actorsByName` `@type` has no trailing prose. Numeric JSON `type: 98` on a first-class AMP frame is coerced via `canonicalTypeName` (do not revert to raw `innerType` — that was the unhandled-generic dump). `FROM_SEED` sets `status = 'seeded'`; `loadWallet({ fromFile: true })` for wallet.json. Still deferred: RFC6902 multi-op JSON bridge (http); Blessed TUI chat markup (terminal, not HTML); `isolatePeerContent` collections; unique Service commit ids; docstring-coverage gate; PR title.
