...
**Last reviewed:** 2026-09-06 — [#187](https://github.com/FabricLabs/fabric/pull/187) vs Hub production plan: **partially adequate** for Beacon ARC / registry / sidechain federation wire (fail-closed patches, contract publish authority, operator identity, Beacon sign rounds). **Not** a full production-plan close: WIP title, MuSig2 epoch aggregate still stub, Hub still owns redeploy pin + Origin-GET redeem + coordinated `contractIdentifier` rename. CodeRabbit leftovers **staged** (watch-only `FABRIC_XPRV` before wallet; env federation threshold independent of validators; typetree MODULE_NOT_FOUND-only; sidechain patch skip witness/AMP when Key has no `.private`). Do not merge as “production complete.”
