...
/**
 * Unified benchmark runner + library exports.
 *
 * - Imported by tests for `runAll/getScenarios/...`
 * - Executed directly by `npm run benchmark` to print table + write
  ...
  return [
    ...
    },
    {
      ...
      asyncFn: async function collectionCreateRow () {
        const cm = collectionMachine();
        await cm.collection.create({ name: cm.machine.sip() });
      }
    }
  ];
}
