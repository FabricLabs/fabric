...
## 2026-08-16
- **Demos landing (Pages):** root `index.html` is the curated `@fabric/core` demos hub (not the old soundtrack.io stub). `examples/home.html` is the flagship literate index. `make:all` runs `make:dev` only (stubs `make:app` / `make:lib` / `make:service` stay opt-in). `make:examples` copies literate HTML into `examples/`. QUICKSTART documents `example:smoke` + `npm run dev` for git checkouts.
...
- **PR #185 review leftovers:** GitHub Pages demos landing is root `index.html`; curated examples index is `examples/home.html`. `report:install` removes `package-lock.json` instead of writing a blank line. C/native pointers go to `examples/README.md`.
