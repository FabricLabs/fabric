...
async function main () {
  ...
  peer.on('peer:candidate', async function (peer) {
    console.log('[EXAMPLES:RELAY]', 'Peer "D" emitted "peer:candidate" event:', peer);
  ...
  console.warn('[EXAMPLES:RELAY]', 'Starting Peer "D"...');
  await peer.start();
  console.log('[EXAMPLES:RELAY]', 'Peer "D" started!');
}
